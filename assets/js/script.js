window.onload = () => {
     ...
     const currentServiceTarget = e.currentTarget;
      ...
      const clearServiceItem = () => {
        serviceContent.forEach(serviceItem => {
          ...
          document.querySelectorAll('.fill1').forEach( (fillColor) => {
            fillColor.style.fill = '#000'; 
          })
        })
      ...
      serviceContent.forEach(serviceElem => {
        serviceElem.addEventListener('click',(e) =>{ 
          e.stopPropagation()
          const currentChannelTarget = e.currentTarget;
          clearServiceItem();

          currentChannelTarget.classList.add("chosen-service");
          currentChannelTarget.querySelector('.fill1').style.fill = '#fff';
          confirmBtn.disabled = false;
          confirmBtn.style.opacity = 1;


          const channelValue = e.currentTarget.getAttribute('value');
  ...
  chosenLocation.forEach(locationItem => {
    locationItem.addEventListener('click',(e) => { 
      ...
      const currentLocationTarget = e.currentTarget;

      currentLocationTarget.classList.add("chosen-location");
      currentLocationTarget.lastElementChild.appendChild(activeLocation);
      currentLocationTarget.lastElementChild.appendChild(map);
